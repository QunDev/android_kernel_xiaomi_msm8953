...
#ifdef CONFIG_KSU
/* tissot/KernelSU-Next: backport path_umount() from 5.9 (needed by the KSU
 * try_umount feature on pre-5.9). Added statically so drivers/kernelsu/Kbuild's
 * build-time sed injection is skipped (its grep guards find these present). */
